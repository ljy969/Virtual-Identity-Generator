...
- **结构合法的证件号码** —— 中国身份证采用真实的 GB 11643-1999（模 11-2）校验算法；银行卡号通过 **Luhn** 校验；**My Number**（官方加权校验，含 `≤1→0` 规则）、**Steuer-ID**（ISO 7064 MOD 11,10）、**NIR**（mod-97 校验键，科西嘉 `2A/2B`→`20/19` 规则）、**Codice Fiscale**（官方月份字母 + 奇偶位校验字符）、**NINO**（排除保留前缀）、**DNI/NIE**（模 23 字母）均遵循各自的公开校验规范（均为示意数据）。
...
- **英国 `nino`** —— 两位字母 + 6 位数字 + 一位后缀字母（`A–D`）；前缀字母表排除官方未使用的字母（`D F I O Q U V`），并排除保留管理前缀（`BG GB KN NK NT TN ZZ`）。
- **日本 `myNumber`** —— 12 位，采用官方校验算法：前 11 位自左向右按 `6,5,4,3,2,7,6,5,4,3,2` 加权；加权和模 11 `≤ 1` 时校验位为 `0`，否则为 `(11 − 和 mod 11) mod 10`。邮编（`123-4567`）不以 `0` 开头。
- **德国 `taxId`（Steuer-ID）** —— 11 位，按 `XX XXX XXX XXX` 分组展示。校验位采用官方 **ISO 7064 MOD 11,10** 递推式（`p=10; s=(p+d)%10（为 0 时取 10）; p=(s*2)%11; 校验位=(11−p)%10`），且首位永不为 `0`。
- **法国 `nir`** —— 按法国 NIR 规范内嵌性别与出生日期，校验键为 `97 − (本体 mod 97)`；省份码 `20` 永不生成（1976 年起未分配），科西嘉使用 `2A`/`2B`，计算校验键时按 INSEE 规则分别替换为 `20`/`19`。
- **意大利 `cf`** —— 采用官方结构的 Codice Fiscale：姓/名辅音、出生年份、月份字母（`A B C D E H L M P R S T`）、日期（女性 +40）、地籍码，以及官方奇/偶位加权校验字符。
- **西班牙 `dni`** —— DNI 风格，采用官方模 23 字母表（`TRWAGMYFPDXBNJZSQVHLCKE`）；约 15% 为 NIE（`X/Y/Z` + 7 位数字），前缀字母按 `0/1/2` 计入最高位参与校验。
- **加拿大 `sin`** —— `XXX-XXX-XXX`，含 **Luhn** 校验位；邮编遵循 `A1B 2C3`。
